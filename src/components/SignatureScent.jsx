...
const SignatureScent = () => {
  ...
  if (hasPreferences) {
    ...
    const favoriteCategory = Object.keys(preferences).reduce((a, b) =>
    ...
    const categoryProducts = products.filter(
      (p) => p.category === favoriteCategory && !cartIds.includes(p._id)
    );

    if (categoryProducts.length > 0) {
      // Prioritize bestsellers first, fallback to first available
      recommendedProduct = categoryProducts.sort(
    ...
    <section className="py-20 bg-[#1a1208] text-white">
      <div className="container-custom">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <h2 className="text-sm font-medium mb-3 text-amber uppercase tracking-[0.2em]">
              AI Intelligence
            </h2>
            ...
            {hasPreferences && recommendedProduct ? (
              <>
                <p className="text-neutral-400 text-lg mb-10 leading-relaxed">
                  ...
                  <strong className="text-amber capitalize">
                    {recommendedProduct.category}
                  </strong>{" "}
                  fragrances, our scent engine has surfaced a masterpiece
                  tailored perfectly to your unique profile.
                </p>
              ...
              <>
                <p className="text-neutral-400 text-lg mb-10 leading-relaxed max-w-lg">
                  Let our intelligent scent engine discover the perfect
                  fragrance that resonates with your unique personality,
                  lifestyle, and physical aura.
                </p>
                <Link
                  to="/scent-finder"
                  className="inline-flex items-center justify-center border-2 border-amber text-amber px-10 py-4 font-medium tracking-wide hover:bg-amber hover:text-white transition-all duration-300 transform hover:-translate-y-1"
                >
                  Launch the Engine
                </Link>
              </>
          ...
          <div className="hidden lg:block relative h-[550px] w-full rounded-2xl overflow-hidden shadow-2xl group">
            ...
            <img
              src={
                recommendedProduct && recommendedProduct.images?.[0]
                  ? getImageUrl(recommendedProduct.images[0])
                  : "/images/tusu.jpg"
              }
              alt="Signature Scent"
            ...
            {recommendedProduct && (
              <div className="absolute bottom-10 left-10 z-20">
                <p className="text-amber uppercase tracking-widest text-xs font-semibold mb-2">
                  98% Match
                </p>
