...
const AILovedSuggestions = ({ currentProduct }) => {
  ...
  const recommendations = products.filter((p) => {
    ...
    const sharedNotes = pTopNotes.filter((note) =>
      currentTopNotes.includes(note)
    );
  ...
  const topRecommendations = recommendations
    .sort(
      (a, b) =>
        b.compatibilityScore - a.compatibilityScore || 0.5 - Math.random()
    )
    .slice(0, 4);
  ...
  return (
    <div className="mt-20 border-t border-neutral-200 pt-16">
      <div className="flex justify-between items-end mb-10">
        <div>
          <h2 className="text-3xl font-serif font-medium text-neutral-900 mb-2">
            People Also Loved
          </h2>
          <p className="text-neutral-500">
            Based on the scent profile of {currentProduct.name}
          </p>
        </div>
      </div>
